...
| 7 | Cross-platform reproducibility diagnosis and correction | **PARTIAL, narrowed to one remaining CI job** — Linux/arm64 (Python 3.11 and 3.12) verified via local Docker; a first real `ubuntu-latest`/`macos-latest` CI run (`33918616460`) failed all three jobs (4 hash divergences + 1 missed pathological-solver test + 1 unrelated CI-timing flake), all five diagnosed and fixed; a SECOND real CI run against the fixed commit (`33948931491`, pushed externally as `9da0bb8`) shows **both `ubuntu-latest` jobs (3.11 and 3.12) now PASS COMPLETELY** — direct confirmation on genuine x86_64 hardware. `macos-latest` failed at a different test (a pipe-deadlock in a pre-existing, unrelated subprocess-lifecycle test) — root-caused (unread `subprocess.PIPE` streams risking a full-buffer write deadlock, not merely a tight timeout) and fixed (`DEVNULL` + 30s margin), re-verified locally, not yet re-confirmed on CI (`gh run rerun` was attempted and blocked by this session's own auto-mode classifier). See `docs/issues/cross-platform-reproducibility.md` for the full evidence and fix record. |
